...
import 'text_back_model.dart';
export 'text_back_model.dart';

class TextBackWidget extends StatefulWidget {
  const TextBackWidget({
    Key? key,
    this.back,
  }) : super(key: key);

  final DocumentReference? back;

  @override
  _TextBackWidgetState createState() => _TextBackWidgetState();
}

class _TextBackWidgetState extends State<TextBackWidget> {
  late TextBackModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => TextBackModel());
              ...
              children: [
                Padding(
                  ...
                  child: FFButtonWidget(
                    onPressed: () async {
                      logFirebaseEvent('TEXT_BACK_COMP__BTN_ON_TAP');
                      logFirebaseEvent('Button_backend_call');

                      var wBackRecordReference = WBackRecord.collection.doc();
                      await wBackRecordReference.set(createWBackRecordData(
                      ...
                      _model.chetsWork = WBackRecord.getDocumentFromData(
                          createWBackRecordData(
                            uid: currentUserReference,
                            set: int.tryParse(_model.setController.text),
                            reps: int.tryParse(_model.repController.text),
                            kg: int.tryParse(_model.kgsController.text),
